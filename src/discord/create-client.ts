...
async function sendRadioAnnouncementNow(
  ...
  try {
    ...
    const payload = {
      ...
      embeds: [new EmbedBuilder().setColor(color).setTitle(`${emoji} ${title}`).setDescription(description).addFields({ name: `${customEmojis.radio} Canal de voz`, value: event.channelId ? `<#${event.channelId}>` : '—', inline: true }, { name: `${customEmojis.music} Gênero`, value: genres, inline: true }, { name: `${customEmojis.audacity} Áudio`, value: status?.audioStatus ?? 'idle', inline: true }, { name: `${customEmojis.loading} Duração`, value: duration, inline: true }, { name: `${customEmojis.music} Música atual`, value: currentTrack ?? 'Metadados não publicados pela emissora', inline: true }, { name: `${customEmojis.info} Último erro`, value: lastError.slice(0, 1024), inline: false }).setFooter({ text: footer }).setTimestamp()],
...
function refreshLivePanels(
  ...
  for (const guildSettings of settings.list()) {
    ...
    sendRadioAnnouncement(client, settings, {
      type: 'playback-start',
